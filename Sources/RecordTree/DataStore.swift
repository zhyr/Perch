...
final class DataStore {
    ...
    func fetchTrees(startMs: Int64, endMs: Int64, limit: Int, offset: Int) throws -> [TreeRec] {
        let rows = try query(
            """
            SELECT id, title, created_at AS ca, updated_at AS ua, archived_at AS aa
            FROM maintree
    ...
    private func latestUpdatedChunk() throws -> (treeID: String, updatedMs: Int64)? {
        let rows = try query(
            "SELECT maintree_id AS tid, updated_at AS ua FROM chunk ORDER BY updated_at DESC, created_at DESC LIMIT 1"
        )
    ...
    private func externalInsert(content: String, attachment: String, source: String, atMs: Int64) throws -> MutationInfo {
        ...
        if let top = try latestUpdatedChunk(), atMs - top.updatedMs <= Config.mergeWindowMs {
            ...
            treeID = newID("tree")
            try run(
                "INSERT INTO maintree (id, title, created_at, updated_at, archived_at) VALUES (?, '', ?, ?, 0)",
                [treeID, atMs, atMs]
            )
        }
    ...
    /// 手工新建一条记录（含首个 chunk）
    func performManualNewRecord(title: String, content: String, atMs: Int64) throws -> MutationInfo {
