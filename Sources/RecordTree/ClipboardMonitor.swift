...
final class ClipboardMonitor {
    ...
    func start() {
        ...
        // 必须加入 .common 模式：默认模式在打开菜单、拖拽/缩放窗口、滚动期间会被挂起，
        // 期间连续发生多次复制时只能捕获到最后一次，造成记录丢失。
    ...
    func stop() {
        ...
        // 重置轮询基准：否则下次 start() 会把「停止期间」的剪贴板变化当成新复制补录
        initialized = false
        lastChangeCount = -1
    }
    ...
    private func tick() {
        ...
        // 2) 敏感/临时内容不入库：密码管理器、浏览器与系统会显式声明这些类型
        //    （org.nspasteboard.* 约定）。跳过可避免把密码、验证码等写入历史。
        if Self.isSensitivePasteboard(pb) { return }
    ...
    /// 标记为「保密 / 临时 / 自动生成」的剪贴板类型（密码管理器、浏览器、系统约定）。
    /// 剪贴板管理器忽略这些内容是通行做法，可避免把密码、验证码等敏感数据写入历史。
    private static let sensitiveTypes: [NSPasteboard.PasteboardType] = [
        NSPasteboard.PasteboardType("org.nspasteboard.ConcealedType"),
        NSPasteboard.PasteboardType("org.nspasteboard.TransientType"),
        NSPasteboard.PasteboardType("org.nspasteboard.AutoGeneratedType"),
        NSPasteboard.PasteboardType("com.agilebits.onepassword"),
        NSPasteboard.PasteboardType("com.agilebits.onepassword4"),
    ]

    private static func isSensitivePasteboard(_ pb: NSPasteboard) -> Bool {
        guard let types = pb.types else { return false }
        return types.contains { sensitiveTypes.contains($0) }
    }
}
